...
export default function MetricsPage() {
	...
	const currentTab = z.enum(['report', 'goals']).parse(searchParams.get('tab') ?? 'report');

	function handleChangeTab(tab: 'report' | 'goals') {
	...
	return (
		<div className="mt-8 flex flex-col gap-4 hiddenOnPhone:px-2">
			<h1 className="text-xl font-semibold tracking-tight lg:text-2xl">
				Acompanhamento da lista de presentes
			</h1>

			<Tabs
				defaultValue="report"
				value={currentTab}
				onValueChange={(value) => handleChangeTab(value as 'report' | 'goals')}
			>
				<TabsOptions />
