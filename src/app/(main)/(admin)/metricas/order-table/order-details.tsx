...
export function OrderDetails({ orderId, open }: IOrderDetailsProps) {
	...
	const orderTotalAmount = useMemo(() => {
		return order?.orderProducts.reduce((amount, product) => {
			return (amount += product.itemQuantity * product.gift.price);
		}, 0);
	}, [order]);

	return (
		<DialogContent className="hiddenOnPhone:overflow-auto">
			<DialogHeader>
					...
					<Table>
						<TableBody>
							...
							<TableRow>
								...
								<TableCell className="flex justify-end">
									{dayjs(order.created_at).format('DD [de] MMMM [às] HH:mm')}
								</TableCell>
							</TableRow>
						</TableBody>
					</Table>
								...
								{order.orderProducts.map((item) => {
									return (
										<TableRow key={item.id}>
											...
											<TableCell className="text-right">
												{(item.gift.price * item.itemQuantity).toLocaleString('pt-BR', {
													style: 'currency',
													currency: 'BRL',
												})}
											</TableCell>
